    ...
    <div className="fixed inset-0 z-[100] flex items-end md:justify-end justify-center pointer-events-none md:pr-2 pr-0 rtl:p-0">
      <div className="bg-white shadow-xl w-full md:max-w-[363px] h-[70vh] max-h-[550px] flex flex-col overflow-hidden pointer-events-auto mb-16 md:mb-20 mb-0 md:mr-0 mr-0 rtl:ml-0 rtl:mr-6 md:rtl:mr-6 md:rounded-lg rounded-t-lg">
        ...
        <div className="overflow-y-auto flex-grow p-4 space-y-3 bg-gray-100">
          {messages.length === 0 && <div className="w-full mt-32 text-gray-500 items-center justify-center flex gap-3">No messages yet.</div>}

          {messages.map((message, index) => (
            <div key={message.id || index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] p-3 rounded-xl shadow ${message.role === "user" ? "bg-pest-red text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border border-gray-200"}`}>
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      code({ node, inline, className, children, ...props }: any) {
                        return inline ? (
                          <code className="bg-gray-200 px-1 rounded text-gray-800">{children}</code>
                        ) : (
                      ...
                      ul: ({ children }) => <ul className="list-disc pl-4 space-y-1 mt-2">{children}</ul>,
                      ol: ({ children }) => <ol className="list-decimal pl-4 space-y-1 mt-2">{children}</ol>,
            ...
            <div className="flex justify-center">
              <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <button className="underline hover:no-underline" type="button" onClick={() => reload()}>
                    Retry
                  </button>
                </div>
              </div>
        ...
        <div className="p-3 border-t flex-shrink-0 bg-white rounded-b-lg">
          <form onSubmit={handleSubmit} className="flex items-center space-x-2">
            <input type="text" value={input} onChange={handleInputChange} placeholder="Type your message..." className="flex-grow border border-gray-300 rounded-lg py-2 px-3 focus:ring-2 focus:ring-pest-red focus:border-pest-red outline-none text-sm" disabled={isLoading} />
            <button type="submit" disabled={!input.trim() || isLoading} className="bg-pest-red text-white p-2 rounded-lg hover:bg-pest-red/90 disabled:bg-gray-300 transition-colors">
